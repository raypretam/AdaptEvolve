...
OUTPUT_ROOT = os.path.join("/", "livecodebench_runs_decision_tree_new_0_200")
...
def _determine_phase1_config(confidence_data: Dict, result: Dict, 
    ...
    if ROUTER_MODEL and not result["solved"]:
        try:
            ...
            # Must match the training data CSV columns
            f_mean = float(confidence_data.get("mean_confidence", 0.0) or 0.0)
            f_bottom = float(confidence_data.get("bottom_window_confidence", 0.0) or 0.0)
            f_tail = float(confidence_data.get("tail_confidence", 0.0) or 0.0)
            f_least = float(confidence_data.get("least_grouped_confidence", 0.0) or 0.0)
            ...
            print(f"[T{threading.current_thread().ident}][{idx+1}/{total}] {safe_id}: "
                  f"Router features - mean: {f_mean:.2f}, bottom: {f_bottom:.2f}, "
                  f"tail: {f_tail:.2f}, least: {f_least:.2f}")
            
            if decision == 1:
                print(f"[T{threading.current_thread().ident}][{idx+1}/{total}] {safe_id}: "
                      f"🔄 Router Decision: SWITCHING to LARGE Config (Class 1)")
                phase1_config_path = CONFIG_LARGE_YAML
                router_decision_label = "Class 1 (Switch Large)"
                phase1_model_type = "large"
            else:
                print(f"[T{threading.current_thread().ident}][{idx+1}/{total}] {safe_id}: "
                      f"➡️ Router Decision: STAYING with SMALL Config (Class 0)")
        except Exception as e:
            print(f"[T{threading.current_thread().ident}][{idx+1}/{total}] {safe_id}: "
                  f"⚠️ Router prediction failed, defaulting to small config. Error: {e}")
            import traceback
            traceback.print_exc()
...
def main():
    ...
    try:
        ...
        num_problems = 280
        df = df.iloc[0:200]  # Get problems 200-399 for this run
