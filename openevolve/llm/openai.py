...
from openevolve.utils.confidence_utils import get_generation_confidence, extract_verbalized_confidence
...
class OpenAILLM(LLMInterface):
    ...
    async def _call_api(self, params: Dict[str, Any]) -> str:
        ...
        #     None, lambda: self.client.chat.completions.create(**params, extra_body={"top_k": 20, "min_p": 0})
        # )
        ...
        confidence = get_generation_confidence(choice)
        verbalized_conf = extract_verbalized_confidence(choice.message.content)
        setattr(choice, "confidence", confidence)
        setattr(choice, "verbalized_conf", verbalized_conf)
        setattr(choice, "llm", params["model"])
        return choice
