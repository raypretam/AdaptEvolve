...
def parse_evolve_blocks(code: str) -> List[Tuple[int, int, str]]:
    ...
    #     return "No EVOLVE-BLOCK found"
    
    # Extract content between markers (excluding the markers themselves)
    end_pos = end_index + len(end_marker)
    extracted_code = code[start_index:end_pos].strip()
...
def parse_full_rewrite(llm_response: str, language: str = "python") -> Optional[str]:
    ...
    if "<think>" in llm_response and "</think>" in llm_response:
        ...
        start_thinking = llm_response.find("<think>")
        end_thinking = llm_response.find("</think>") + len("</think>")
        llm_response = llm_response[:start_thinking] + llm_response[end_thinking:]
